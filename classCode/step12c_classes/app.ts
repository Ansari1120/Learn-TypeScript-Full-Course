...
class C {
    private  _length = 0; // encapsulated in class C
    get length(){
        return this._length // to access outside of class get the value
    }
    set length(value){
       this._length = value;  // to set new value of accessed value
    }
}const a = new C();
a.length; // getter
a.length = 3; // setter